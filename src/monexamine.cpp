...
void bandage_animal( monster &z )
{
    ...
    if( !wieldede_item ) {
        add_msg( _( "You need to have in hand something to stop the bleeding." ), z.name() );
        return;
    }
    const use_function *usage = wieldede_item.get_item()->type->get_use( "heal" );
    if( usage == nullptr ) {
        add_msg( _( "You aren't going to stop %s bleeding with this %s." ), z.name(),
    ...
    if( actor->bleed ) {
        ...
        add_msg( _( "The %s is no longer bleeding." ), z.name() );
...
bool monexamine::pet_menu( monster &z )
{
    ...
    if( z.has_flag( mon_flag_PET_MOUNTABLE ) && player_character.can_mount( z ) ) {
        ...
        amenu.addentry( mount, false, 'r', _( "You require 1 rank of ecology to ride" ) );
        ...
        amenu.addentry( mount, false, 'r', _( "You require 4 ranks of ecology to ride without a saddle" ) );
    }
    amenu.query();
